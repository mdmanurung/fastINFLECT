...
#'
#' @description Unimodality score is calculated for all calculated
#' metaclusterings. This is the central fast reimplementation of the original
#' INFLECT QC sweep: each distinct SOM-node subtree is evaluated once, then the
#' per-k accuracy matrices are assembled from that cache. The marker-level
#' pass/fail definition remains aligned with the original `FlowSOMQC()` method.
#'
...
iteration.QC <- function(FlowSOM.results,
  ...
  unique_members <- list()
  for (Ui in set.i) {
    mc <- metaclustering.list[[as.character(Ui)]]
    n_clusters <- max(mc)
