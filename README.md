...
fastINFLECT is not on CRAN. The simplest installation path uses `pak`, which
also resolves the Bioconductor dependency on FlowSOM:
...
The diagnostic curve is available as `result$ggplot`. Its selection table
distinguishes tested partitions from fitted estimates:
...
Only rows with `partition_available = TRUE` have a corresponding entry in
`result$metaclustering.list`. A fitted inflection with
`k_status = "fitted_estimate_no_partition"` remains an estimate until that
literal k is explicitly evaluated.

The pass-rate curve alone should not be used to declare a selected partition
unimodal. Inspect the separate criterion matrices and, for consequential
...
On platforms without fork support, the same arguments are validated and a
recorded serial fallback is used. Simulated dip p-values and optional
...
The repository includes a resumable audit for the 39,050,953-event BMV model:

```bash
data-raw/submit-real-model-modality.sh
```

It materialises every requested Ward and FlowSOM control partition, stages
nested deterministic event samples, runs dip and ACR tests with
Benjamini-Hochberg correction within each 27-marker cluster family, repeats
flagged and audited pairs at 1,000 and 5,000 events, and writes checkpointed
evidence under `inst/benchmarks/real-model-modality/`.

For the frozen BMV model (`md5 784b3a6048c379ca64bbe8d4c0fb063d`),
the literal full-event k = 25,...,100 comparison gives a combined INFLECT
...
[GarciaVallejoLab repository](https://github.com/jnverhoeff/GarciaVallejoLab).
The exported `INFLECT()` name is retained for API continuity. Memoised
hierarchy cuts, marker-at-a-time indexed scoring, deterministic sampling, and
compiled accelerators provide the faster implementation.
