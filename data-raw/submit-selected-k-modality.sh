...
array_limit=${INFLECT_ARRAY_LIMIT:-64}
job_script="${repo_root}/data-raw/selected-k-modality.sbatch"
work_dir=${INFLECT_MODALITY_WORKDIR:-${repo_root}/data-raw/.selected-k-modality-work}
...
stage_raw=$(sbatch \
  --parsable \
  --chdir="${repo_root}" \
  --job-name=inflect-selected-stage \
  --time=12:00:00 \
  --mem=96G \
...
pilot_raw=$(sbatch \
  --parsable \
  --chdir="${repo_root}" \
  --job-name=inflect-selected-pilot \
...
base_raw=$(sbatch \
  --parsable \
  --chdir="${repo_root}" \
  --job-name=inflect-selected-base \
  --dependency="afterok:${stage_id}" \
...
classify_raw=$(sbatch \
  --parsable \
  --chdir="${repo_root}" \
  --job-name=inflect-selected-classify \
...
refine_raw=$(sbatch \
  --parsable \
  --chdir="${repo_root}" \
  --job-name=inflect-selected-refine \
...
summarize_raw=$(sbatch \
  --parsable \
  --chdir="${repo_root}" \
  --job-name=inflect-selected-summarize \
...
printf 'Submitted selected-k modality workflow:\n'
sed 's/^/  /' "${manifest}"
