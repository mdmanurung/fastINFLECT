...
The historical pre-release `vignette_cache.rds` is retained as
engineering performance material. It used `zeroes.in = FALSE` and an
aggregate pass-rate interpretation, so it does not support claims about
cluster modality.
