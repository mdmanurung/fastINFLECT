# Using fastINFLECT 1.0.0

fastINFLECT 1.0.0 uses literal cluster-count schedules, retains complete
transformed marker distributions by default, and reports dip and IQR
evidence separately. It also distinguishes fitted estimates from
partitions that were materialised.
...
Candidate values enter through the required `set.i` argument. It must
contain at least five unique, strictly increasing integer values, and no
value may exceed the number of SOM nodes.
...
With the schedule defined, `zeroes.in = TRUE` retains negative, zero,
and positive finite values. This matters for centred or otherwise
transformed marker data because dropping non-positive measurements
changes the tested distribution.
...
Conversely, `zeroes.in = FALSE` excludes every non-positive value, warns
when negative inputs are present, and records the exclusions:
...
Under either `zeroes.in` setting, no zero-valued padding observations
are manufactured.

## Read separated criteria

`uniform.test` selects the aggregate pass-rate curve. All component
...
The canonical aggregate column is `qc_pass_rate`. Compatibility aliases
from the original INFLECT interface remain available but are deprecated:
...
Of these, “no detected multimodality” is deliberately weaker than
“unimodal.” The dip test can reject a unimodal null; failure to reject
does not prove that null.
