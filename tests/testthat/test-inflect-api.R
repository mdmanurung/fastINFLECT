test_that("INFLECT forwards QC arguments and returns an inflect.results object", {
  ...
  env$iteration.metacluster <- function(FlowSOM.results, set.i) {
    stats::setNames(
  ...
  env$QC.to.curve <- function(collection.U) {
    list(
  ...
  expected_set_i <- 7:11
  expect_false("zeroes.in" %in% names(formals(env$INFLECT)))
  expect_error(env$INFLECT(fake), "`set.i` is required")
  expect_error(
    env$INFLECT(fake, set.i = 5:9, workers = 0L),
    "`workers`"
  )
  result <- env$INFLECT(
    fake,
  ...
  expect_identical(captured$workers, 2L)
  expect_equal(captured$markers, "CD4")
  ...
  expect_false("zeroes.in" %in% names(result$provenance))
  expect_equal(result$provenance$marker_selection, "explicit")
  expect_equal(result$provenance$requested_markers, "CD4")
...
test_that("INFLECT rejects removed zero handling and non-finite data early", {
  env <- new.env(parent = globalenv())
  source_pkg_file("som-adapter.R", envir = env)
  source_pkg_file("inflect-qc-core.R", envir = env)
  source_pkg_file("inflect-provenance.R", envir = env)
  ...
  expect_error(
    env$INFLECT(fake, set.i = 5:9, zeroes.in = TRUE),
    "unused argument.*zeroes.in"
  )

  fake$data[1, "CD3"] <- NA_real_
  fake$data[2, "CD3"] <- Inf
  metaclustering_started <- FALSE
  env$iteration.metacluster <- function(...) {
    metaclustering_started <<- TRUE
    stop("metaclustering should not start", call. = FALSE)
  }

  expect_error(
    env$INFLECT(fake, set.i = 5:9),
    "only finite values.*CD3 \\(2\\)"
  )
  expect_false(metaclustering_started)
})
